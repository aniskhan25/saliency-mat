...
void mexFunction(int output_size, mxArray *output[], int input_size, const mxArray *input[])
{
    ...
    if (faces->total)
    {
        output[0] = mxCreateDoubleMatrix(faces->total, 5, mxREAL);
        Data = mxGetPr(output[0]); // Get the pointer to output variable
        // Iterate trou each of the detected faces
        for( i = 0; i < faces->total; i++ )
        {
                    CvAvgComp r = *((CvAvgComp*) cvGetSeqElem( faces, i ));
                    Data [i] = r.rect.x;
                    Data [i+faces->total*1] = r.rect.y;
                    Data [i+faces->total*2] = r.rect.width;
                    Data [i+faces->total*3] = r.rect.height;
                    Data [i+faces->total*4] = r.neighbors;           
        }
    }
    else
    {
